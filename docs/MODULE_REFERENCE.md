...
| `hyperparameter_tuning/parameters.py` | The values each hyperparameter may take, one class per Optuna `suggest_*` call, validated when created | `Parameter`, `FloatParameter`, `IntParameter`, `CategoricalParameter` | — (optuna, pydantic only) | [Hyperparameter tuning plan](HYPERPARAMETER_TUNING_PLAN.md) |
| `hyperparameter_tuning/evaluator.py` | Score one Optuna trial: set its parameters, then for every CV fold fit a `FeatureTransformer` and a `BaseAgeGroupModel` on the training rows and score the validation rows with a `Metric`; combine the fold scores | `CVHyperparameterEvaluator`, `Aggregation` | `utils`, `feature_engineering`, `modeling`, `scoring`, `hyperparameter_tuning.parameters` | [Hyperparameter tuning plan §4.2](HYPERPARAMETER_TUNING_PLAN.md) |
