...
| `base.py` | Shared lifecycle: `fit`/`predict`/`evaluate`, fitted preprocessing, training hashes, metadata, state bundles | `BaseAgeGroupModel` (`fit`, `predict`, `evaluate`, `to_state_bundle`, `from_state_bundle`, `configuration_record`) | `feature_engineering`, `evaluation`, `metrics`, `results`, `hashing`, `state_bundle` |
