...
manifest is refused. Replay verifies the table and schema hashes and exact ID
partition before materializing the split. Never generate a replacement split
...
buildings, clamped to that range, so the realized holdout fraction can differ
from the requested fraction in either direction; small neighborhoods push it
up, and singletons pull it down.
...
`age_group_prediction.experiment`.
...
- `DirectCohortModel`: separate LightGBM count models for cohort outcomes.
...
intervals = neighborhood_cluster_bootstrap(
    observed_df,
    prediction,
    metrics,
...
replicates, confidence level, failure tolerance, and PIT settings.
...
predictions, calibration, permutation importance, fold coverage, selections,
...
conditional candidates using comparable joint likelihood evidence. A freeze
is immutable; transitions return a new value.
...
bundle as a models-from-code pyfunc whose input is a target-free raw modeling
frame and whose output is `building_id`, `total_mean`, and each cohort's
`{cohort}_mean` and `{cohort}_probability` columns.
...
The project default seed is 42. Split APIs accept NumPy generators and record
whether the caller supplied one. Cross-validation requires either
